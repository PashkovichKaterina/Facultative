...
public class ArchiveDAOImpl implements ArchiveDAO {
    ...
    @Override
    public void insertArchive(int studentId, int courseId, int averageMark, Date startDate, Date endDate, String review)
            throws DAOException {
        try {
    ...
    public Set<ArchiveCourse> getArchiveCourseByStudentOnRu(int studentId) throws DAOException {
        try {
            ...
            try (PreparedStatement statement = connection.prepareStatement(SqlQuery.GET_ARCHIVE_COURSE_BY_STUDENT_RU)) {
                statement.setInt(1, studentId);
                try (ResultSet resultSet = statement.executeQuery()) {
    ...
    public Set<ArchiveCourse> getArchiveCourseByStudentOnEn(int studentId) throws DAOException {
        try {
            Connection connection = BaseConnectionPool.getInstance().getConnection();
            Set<ArchiveCourse> archives = new HashSet<>();
            try (PreparedStatement statement = connection.prepareStatement(SqlQuery.GET_ARCHIVE_COURSE_BY_STUDENT_EN)) {
                statement.setInt(1, studentId);
                try (ResultSet resultSet = statement.executeQuery()) {
                    while (resultSet.next()) {
                        ArchiveCourse archiveCourse = new ArchiveCourse();
