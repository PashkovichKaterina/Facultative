                ...
                <c:if test="${marks != null && marks.size() != 0}">
                    <table class="table simulacion mx-auto">
                        <tr>
                            ...
                            <c:forEach var="num" items="${works}">
                                <th class="table-student">${num} <i class="fa fa-pencil" aria-hidden="true"></i></th>
                            </c:forEach>
                        </tr>
                        <tbody>
                        <c:set var="i" value="${1}"/>
                        <c:forEach var="num" items="${marks.entrySet()}">
                            <tr>
                                ...
                                <td>
                                    <a href="#">
                                            ${num.getKey().getSurname()} ${num.getKey().getName()} ${num.getKey().getPatronymic()}
                                    </a>
                                </td>
                                <c:forEach var="mark" items="${num.getValue()}">
                                    <td>${mark}</td>
                                </c:forEach>
                            </tr>
            ...
            <div id=content-2 class="student-table-change">
                ...
                <c:if test="${classes != null && classes.size() != 0}">
                    <table class="table">
                        ...
                        <tbody>
                        <c:set var="i" value="${1}"/>
                        <c:forEach var="num" items="${classes.entrySet()}">
                            <tr>
                                <th scope="row">${i}</th>
                                <td>${num.getKey()}</td>
                                <td>${num.getValue()}</td>
